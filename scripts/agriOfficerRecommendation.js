...
function addAssignmentRow({ officerID, farmerID, recommendation, joiningDate, transferDate }) {
  const tr = document.createElement("tr");
  tr.classList.add("hover:bg-gray-50");

  [officerID, farmerID, recommendation, joiningDate, transferDate].forEach(text => {
...
form.addEventListener("submit", function (e) {
  ...
  if (!officerID || !farmerID || !recommendation || !joiningDate || !transferDate) {
    alert("Please fill out all fields.");
    return;
  }

  addAssignmentRow({ officerID, farmerID, recommendation, joiningDate, transferDate });
